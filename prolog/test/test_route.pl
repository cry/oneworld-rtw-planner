...
test(x_prefix_marks_a_transfer_and_bare_codes_are_stopovers) :-
    ...
    assertion(Segs = [ rseg(1, flight, lhr, jfk, _, _, _, _, _, transfer, _),
                       rseg(2, flight, jfk, lax, _, _, _, _, _, stopover, _),
                       rseg(3, flight, lax, lhr, _, _, _, _, _, unknown, _) ]).
...
test(city_codes_pass_through_the_parser_unresolved) :-
    ...
    assertion(Segs = [ rseg(1, _, nyc, lon, _, _, _, _, _, transfer, _),
                       rseg(2, _, lon, sin, _, _, _, _, _, unknown, _) ]).
...
test(three_letter_codes_starting_with_x_are_places) :-
    ...
    assertion(Segs = [ rseg(1, _, hkg, xiy, _, _, _, _, _, stopover, _),
                       rseg(2, _, xiy, pek, _, _, _, _, _, unknown, _) ]).
...
test(double_slash_makes_a_surface_sector) :-
    ...
    assertion(Segs = [ rseg(1, surface, bkk, sin, _, _, _, _, _, _, _),
                       rseg(2, flight, sin, lhr, _, _, _, _, _, _, _) ]).
...
test(two_letter_tokens_are_the_carrier_of_the_following_leg) :-
    ...
    assertion(Segs = [ rseg(1, _, lhr, jfk, ba, ba, _, _, _, _, _),
                       rseg(2, _, jfk, lax, aa, aa, _, _, _, _, _) ]).
