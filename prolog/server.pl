...
%
% Every asset is held as a list of bytes rather than as text, so one code path
% serves the stylesheet and the fonts alike and nothing has to reason about the
% encoding of a woff2. Failing here fails the build, which is where a missing UI
% file should be noticed.
%
% web/app.css and web/map.js are generated by `npm run build` from app.src.css and
% map.src.js and committed; see the notes in those files. Nothing in the image
% knows about npm.
...
:- prolog_load_context(directory, Dir),
   ...
   forall(asset_file(File, Path, Type),
          ( atomic_list_concat([Web, File], Abs),
            read_file_to_codes(Abs, Bytes, [encoding(octet)]),
...
% One prefix handler for the font directory. It resolves nothing from the
% filesystem -- the path is looked up in the table above and anything absent is a
% 404 -- so there is no traversal to defend against.
...
% they are immutable. The page, the stylesheet and the script are rebuilt under
% the same three names, so they are revalidated every load; at these sizes, over
% a connection that is usually loopback, that costs nothing worth optimising.
...
from_disk(Path, Bytes) :-
    ...
    catch(read_file_to_codes(Abs, Bytes, [encoding(octet)]), _, fail).
