...
Node is needed only to rebuild the UI stylesheet, and only if you edit it: `web/app.css` is
generated and committed. Running, testing and deploying the validator never touch npm — see
[Web UI](#web-ui).
...
**Building the stylesheet.** `web/app.css` is generated by Tailwind from
[`web/app.src.css`](web/app.src.css) and **committed**, for the same reason
...
```sh
npm install                # once
npm run css                # rebuild web/app.css after editing classes
npm run css:watch          # or leave it running while you work
...
without that style — `npm run css && git diff --exit-code web/app.css` in CI catches it.
...
5. **Serialization and HTTP round trip** — the JSON body must report the same verdict and rule ids
   ...
   size, segment and timeout guards, and the static assets: a stylesheet that 404s leaves the UI
   rendering as unstyled markup rather than failing loudly, and a font re-encoded on the way out
   arrives corrupt with no error raised, so it is compared byte for byte against what the server
   holds.
